...
**Output:** Updated `src/styles.css` and `tailwind.config.ts`
**Duration:** 5 minutes

**Note:** This project uses Tailwind CSS v4 with CSS-first configuration (not v3 JS config).
...
```typescript
// tailwind.config.ts - minimal, theme in @theme inline
import type { Config } from 'tailwindcss';

const config: Config = {
  darkMode: ['class'],
  content: ['./src/**/*.{ts,tsx,html}'],
  // Typography plugin configured via @plugin in styles.css (v4 approach)
};
...
```typescript
export type MarkerType = 'code_block' | 'math_block' | 'link' | 'bold' | 'italic' | 'strikethrough';
