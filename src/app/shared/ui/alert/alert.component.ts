...
const alertVariants = cva(
  'relative w-full rounded-lg border text-sm grid has-[>svg]:grid-cols-[auto_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current',
  {
    variants: {
      variant: {
...
export class AlertDescriptionComponent {
  ...
  protected computedClass = computed(() => {
    return cn(
      'text-muted-foreground col-start-2 text-sm leading-relaxed break-words',
      this.class()
    );
  });
}
