...
export type DropdownMenuContentAlign = 'start' | 'center' | 'end';
export type DropdownMenuContentSide = 'top' | 'right' | 'bottom' | 'left';

@Component({
  selector: 'div[spark-dropdown-menu-content]',
  ...
  styles: `
    :host {
      ...
      z-index: 50;
      min-width: 8rem;
      max-width: var(--radix-dropdown-menu-content-available-width, var(--radix-popper-anchor-width));
      max-height: var(--radix-dropdown-menu-content-available-height, var(--radix-popper-anchor-height));
      overflow: auto;
    }
  `,
})
export class DropdownMenuContentComponent {
  ...
  readonly align = input<DropdownMenuContentAlign>('start');
  readonly sideOffset = input<number>(4);
  readonly class = input<string>('');
  readonly side = input<DropdownMenuContentSide>('bottom');
  ...
  private getEnabledItems(): DropdownMenuItemDef[] {
    return this.root.items().filter((item: DropdownMenuItemDef) => !item.disabled);
  }
