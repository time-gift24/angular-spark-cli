 ...
 *
 * Usage:
 * <div spark-navmenu-root [(open)]="isOpen">
 *   <button spark-navmenu-trigger>
 *     Open Menu
 *   </button>
 *   <div spark-navmenu-content>
 *     <div spark-navmenu-item>Item 1</div>
 *   </div>
 * </div>
 */
...
export const NAV_MENU_ROOT = new InjectionToken<NavMenuRootToken>('NAV_MENU_ROOT');

export interface NavMenuRootToken {
  ...
  readonly items: Signal<NavMenuItemDef[]>;
  readonly registerItem: (item: NavMenuItemDef) => void;
  readonly unregisterItem: (value: string) => void;
  readonly registerCheckboxItem: (item: NavMenuCheckboxItemDef) => void;
  readonly unregisterCheckboxItem: (value: string) => void;
  readonly radioGroups: Signal<Map<string, NavMenuRadioGroupDef>>;
  readonly registerRadioGroup: (value: string, group: NavMenuRadioGroupDef) => void;
...
export interface NavMenuCheckboxItemDef {
  value: string;
...
export interface NavMenuRadioGroupDef {
  value: Signal<string | null>;
  items: Signal<NavMenuItemDef[]>;
  registerItem: (item: NavMenuItemDef) => void;
  unregisterItem: (value: string) => void;
}

@Component({
  selector: 'div[spark-navmenu-root]',
  ...
  providers: [
    {
      provide: NAV_MENU_ROOT,
      useExisting: NavMenuRootComponent,
    },
  ],
})
export class NavMenuRootComponent implements NavMenuRootToken {
  ...
  readonly items = signal<NavMenuItemDef[]>([]);
  readonly checkboxItems = signal<Map<string, NavMenuCheckboxItemDef>>(new Map());
  readonly radioGroups = signal<Map<string, NavMenuRadioGroupDef>>(new Map());
