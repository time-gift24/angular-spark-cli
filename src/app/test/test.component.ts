/**
 * Test Component for Streaming Markdown
 *
 * This component provides a test harness for the streaming markdown feature.
 * It integrates MockAIApi service and provides UI controls to start, stop, and reset streaming.
 *
 * Phase 8 Implementation:
 * - Task 8.2: Define TestComponent interface with streaming controls
...
import { StreamingMarkdownComponent } from '../shared/components/streaming-markdown/streaming-markdown.component';
 ...
 * Test component for streaming markdown.
 *
 * This component serves as both a test harness and demo page for the streaming
...
export class TestComponent {
  ...
  startStreaming(): void {
    ...
    if (this.streamControl.isActive()) {
      console.log('[TestComponent] Streaming already active');
      return;
    }

    console.log('[TestComponent] Starting markdown stream...');
    ...
    console.log('[TestComponent] Stream initialized and bound to component');
  }
  ...
  stopStreaming(): void {
    if (!this.streamControl.isActive()) {
      console.log('[TestComponent] No active stream to stop');
      return;
    }

    console.log('[TestComponent] Stopping markdown stream...');
    ...
    console.log('[TestComponent] Stream stopped');
  }
   ...
   */
  reset(): void {
    console.log('[TestComponent] Resetting streaming state...');
    this.streamControl.reset();
    this.isStreaming = false;
    this.aiStream$ = EMPTY;
    this.rawMarkdown = '';

    console.log('[TestComponent] State reset complete');
  }
  ...
  startStreamingWithPattern(pattern: StreamPattern): void {
    if (this.streamControl.isActive()) {
      console.warn('[TestComponent] Cannot start new stream: already active');
      return;
    }

    console.log('[TestComponent] Starting stream with custom pattern:', pattern);
    this.streamControl.start();
    this.isStreaming = true;

    this.aiStream$ = this.mockApi.streamMarkdownWithPattern(pattern);

    console.log('[TestComponent] Custom pattern stream initialized');
  }
